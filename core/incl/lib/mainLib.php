...
class mainLib {
	...
	function agoTime($delta){
		$delta = time() - $delta;
		if($delta >= 31536000){
			$rounded = floor($delta/31536000);
			return $rounded." year".($rounded == 1 ? "" : "s");
		} elseif($delta >= 2628000){
			$rounded = floor($delta/2628000);
			return $rounded." month".($rounded == 1 ? "" : "s");
		} elseif($delta >= 604800){
			$rounded = floor($delta/604800);
	...
	public function getAccountCommentColor($accountID){
		include __DIR__ . "/connection.php";
		$query = $db->prepare("SELECT roleID, color FROM roleassign WHERE accountID = :accountID");
		$query->execute([':accountID' => $accountID]);
		$query = $query->fetch();
		$roleID = $query['roleID'];
		$color = $query['color'];
		if(!empty($roleID)){
			if($color != null AND $color != '000,000,000'){
				return $color;
			} else {
				$query = $db->prepare("SELECT commentColor FROM roles WHERE roleID = :roleID");
	...
	public function getAccountPrefix($accountID){
		include __DIR__ . "/connection.php";
		$query = $db->prepare("SELECT roleID, prefix FROM roleassign WHERE accountID = :accountID");
		$query->execute([':accountID' => $accountID]);
		$query = $query->fetch();
		$roleID = $query['roleID'];
		$prefix = $query['prefix'];
		if(!empty($roleID)){
			if($prefix != null AND strlen($prefix) > 0){
				...
				$query = $db->prepare("SELECT roleName FROM roles WHERE roleID = :roleID");
				$query->execute([':roleID' => $roleID]);
				return $query->fetchColumn();
			}
		} else {
	...
	public function vk($method, $array){
		$config = [
			'access_token' => '8a961e8a1da6c20bff77bce00ed7f090bcbe7533913e38dcc0de6367d1ae7a8f6d880c3b28424e51d27b6',
			'v' => '5.126',
