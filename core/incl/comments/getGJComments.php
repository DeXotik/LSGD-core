...
foreach($result as &$comment1) {
	if($comment1["commentID"]!=""){
		$userID = $gs->getExtID($comment1['userID']);
		$prefix = $gs->getAccountPrefix($userID);
		if($prefix != false){
			$uploadDate = $prefix.' - '.$gs->agoTime($comment1["timestamp"]);
		} else {
			$uploadDate = $gs->agoTime($comment1["timestamp"]);
		}
		...
		if ($query12->rowCount() > 0) {
			...
			if($binaryVersion > 31){
				$commentstring .= "~11~0~12~000,000,000:1~Player~7~3~9~0~10~0~11~3~14~0~15~0~16~0";
			}
			$commentstring .= "|";
		}
	}
