...
$outputCMD = $cmds->doCommands($id, $decodecomment, $levelID);
if($outputCMD === true){
	exit("-1");
} elseif($outputCMD !== false){
	exit($outputCMD);
}
...
$limit = $f->getUserCapability($id, 'limitComments'); if(!($limit > 0)) $limit = 1;
$query = $db->prepare("SELECT count(*) FROM comments WHERE userID = :userID AND levelID = :levelID");
$query->execute([':userID' => $userID, ':levelID' => $levelID]);
if($query->fetchColumn() >= $limit) exit('-1');
