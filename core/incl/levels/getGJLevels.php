...
switch($type){
	...
	case 16: //HALL OF FAME
		...
		break;
	case 7: //MAGIC
		$suggests = $db->prepare("SELECT suggestLevelId, timestamp FROM suggest GROUP BY suggestLevelId ORDER BY timestamp DESC");
		$suggests->execute(); $suggests = $suggests->fetchAll();
		$suggestLevels = '';
		foreach($suggests AS $suggest){
			$suggestLevels .= $suggest['suggestLevelId'].',';
		}
		$suggestLevels = substr($suggestLevels, 0, -1);
		$params[] = "levelID IN ($suggestLevels) AND starStars = 0";
		break;
	case 10: //MAP PACKS
		$order = false;
		$params[] = "levelID IN ($str)";
