...
$f = "SELECT count(*) FROM users WHERE stars > :stars AND isBanned = 0 AND lastPlayed > :time AND isRegistered = 1"; //I can do this, since I already know the stars amount beforehand
$query = $db->prepare($f);
$query->execute([':stars' => $user["stars"], ':time' => time()-7776000]);
if($query->rowCount() > 0){
